...
def test_Method():
    ...
    result = cross_validate(lambda random_state: TestMethod(), data, ["x"],
                            get_splits=partial(get_train_test, test_size=0.5), random_state="y", n_repeats=, n_jobs=1)
    ...
    assert_series_equal(
        result["train_test"]["y_target"],
        Series([140.0, 180.0, 160.0, 130.0, 190.0], index=[4, 8, 6, 3, 9]),
    )


def test_Method_suggest_optuna():

    class TestStep1(Method):

        @staticmethod
        def suggest_optuna(trial: Trial, prefix: str = "") -> Tuple[Trial, Dict]:
            return trial, {
                f"{prefix}_hyperparameter1": trial.suggest_categorical(
                    f"{prefix}_hyperparameter", ["a", "b", "c"]
                )
            }

    class TestStep2(Method):

        @staticmethod
        def suggest_optuna(trial: Trial, prefix: str = "") -> Tuple[Trial, Dict]:
            return trial, {
                f"{prefix}_hyperparameter2": trial.suggest_categorical(
                    f"{prefix}_hyperparameter2", ["a", "b", "c"]
                )
            }
...
def test_apply_to_model():
    results = [
        {
            "s1": {"model": {"internal": [1, 2, 3]}},
            "s2": {"model": {"internal": [4, 5, 6]}},
        },
        {
            "s1": {"model": {"internal": [7, 8, 9]}},
        },
    ]
