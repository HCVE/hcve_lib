...
def sample_data():
    ...
    df = pd.DataFrame({"feature1": range(10), "feature2": range(10)}, index=range(10))
    return df


def test_get_boostrap_basic_functionality(sample_data):
    """Test basic output structure and sizes"""
    result = get_bootstrap(sample_data, None, random_state=42)
...
def test_get_boostrap_reproducibility(sample_data):
    """Test if results are reproducible with same random_state"""
    result1 = get_bootstrap(sample_data, None, random_state=42)
    result2 = get_bootstrap(sample_data, None, random_state=42)

    assert result1["bootstrap"] == result2["bootstrap"]


def test_get_boostrap_different_seeds(sample_data):
    """Test if different random states give different results"""
    result1 = get_bootstrap(sample_data, None, random_state=42)
    result2 = get_bootstrap(sample_data, None, random_state=43)
