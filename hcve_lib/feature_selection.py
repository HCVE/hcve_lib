...
def get_importance_feature_selection_curve(X, y, get_results, max_features: int = None):
    feature_selection_curve_all = evaluate_n_features(
        len(X.columns), X, y, get_results, return_result=True
    )
...
def evaluate_n_features(
    n_features, X, y, get_results, metrics=None, fi=None, return_result=False
):
    print(n_features)
    if fi is None:
