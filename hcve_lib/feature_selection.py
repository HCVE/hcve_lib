...
from hcve_lib.utils import partial, NonDaemonPool
from hcve_lib.utils import transpose_dict


def get_feature_selection_curve(X, y, get_results, max_features: int = None):
    feature_selection_curve_all = evaluate_n_features(len(X.columns), X, y, get_results, return_result=True)
    ...
    feature_selection_curve = dict(
        zip(
            n_feature_range,
            map(
                partial(evaluate_n_features, X=X, y=y, get_results=get_results, fi=fi),
                n_feature_range,
            )
        )
    )
...
def evaluate_n_features(n_features, X, y, get_results, metrics=None, fi=None, return_result=False):
    print(n_features)
    if fi is None:
    ...
    results = get_results(X=X_selected, y=y)
    ...
    output = dict(metrics=metrics_value, features=X_selected.columns.tolist())
    if return_result:
        output['results'] = results
    return output


def plot_feature_selection_curve(feature_selection_curve, metric_name: str = None):
    ...
    for metric_name, values_per_n in per_metric.items():
        ...
        fig.update_layout(title=metric_name, xaxis_title='n features', yaxis_title=metric_name)
